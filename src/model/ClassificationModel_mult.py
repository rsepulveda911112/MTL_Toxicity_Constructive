...
class ClassificationModelMult:
    ...
    def evaluate(
        ...
        preds = np.empty((len(eval_dataset), self.labels_list[task_id]))
        if multi_label:
            out_label_ids = np.empty((len(eval_dataset), self.labels_list[task_id]))
        else:
    ...
    def predict(self, to_predict, multi_label=False, task_id=None):
        ...
        eval_loss = 0.0
        nb_eval_steps = 0
        preds = np.empty((len(to_predict), self.labels_list[task_id]))
        if multi_label:
            out_label_ids = np.empty((len(to_predict), self.labels_list[task_id]))
        else:
